...
class GUI2Dextended:
    def __init__(self, master):
        ...
        self.info_funcs = [self.info10,self.info11,self.info12,self.info13,self.info14,self.info15]
        self.info_buttons =["self.btn_sf","self.btn_abt","self.btn_abn","self.btn_OARlimit","self.btn_tumorlimit","self.btn_BED"]
        ...
        self.example_list = ["sparing factors separated by space",10,3,90,72,"only needed if we calculate the dose for a single fraction"]
        ...
        self.btn_BED = tk.Button(master = self.frm_form,text = '?',command = self.info_funcs[5])
        self.btn_BED.grid(row=5,column=2)

        self.ent_BED.configure(state = 'disabled') #the standard calculation is a whole plan calculation
        self.ent_tumorlimit.configure(state = 'disabled')


        self.btn_compute = tk.Button(master=self.frm_buttons, text="compute plan", command = self.compute_plan)
        ...
        self.chk_OARswitch = tk.Checkbutton(master = self.frm_buttons, text = "Calculate optimal plan by minimizing OAR and aiming on prescribed tumor dose", variable = self.var_OAR,onvalue = 1,offvalue = 0,command = self.OAR_enable)
...
#assign infobutton commands
    ...
    def info15(self):
        self.lbl_info["text"] = 'Insert the accumulated BED so far. (If fraction one, it is zero).'
        ...
        if self.var.get() == 0:
            ...
            try:
                ...
                if self.var_OAR.get() == 0:
                    ...
                    self.lbl_info["text"] = f"The optimal dose for fraction {len(sparing_factors)-1},  = {physical_dose}\n dose delivered to tumor = {tumor_dose}\n dose delivered OAR = {OAR_dose}"
            except ValueError:
                self.lbl_info["text"] = "please enter correct values\nsparing factors have to been inserted with space in between. No additional brackets needed."
